...
describe('+ AuthController', () => {
  ...
  // Before all tests, clear existing data and create a mock user
          ...
          .then(() => {
            request.post('/users')
              ...
              .end((requestErr, res) => {
                ...
                expect(res.body.email).to.eql(mockUser.email); // Validate user creation
    ...
    it('+ Fails with no "Authorization" header field', function (done) {
      ...
      request.get('/connect')
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.deep.eql({ error: 'Unauthorized' }); // Validate error response
          done();
        });
    });
    ...
    it('+ Fails for a non-existent user', function (done) {
      ...
      request.get('/connect')
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.deep.eql({ error: 'Unauthorized' }); // Validate error for invalid credentials
          done();
        });
    });
    ...
    it('+ Fails with a valid email and wrong password', function (done) {
      ...
      request.get('/connect')
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.deep.eql({ error: 'Unauthorized' }); // Validate error for wrong password
          done();
        });
    });
    ...
    it('+ Fails with an invalid email and valid password', function (done) {
      ...
      request.get('/connect')
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.deep.eql({ error: 'Unauthorized' }); // Validate error for invalid email
          done();
        });
    });
    ...
    it('+ Succeeds for an existing user', function (done) {
      ...
      request.get('/connect')
        ...
        .end((err, res) => {
          ...
          expect(res.body.token).to.exist; // Validate token generation
  ...
  describe('+ GET: /disconnect', () => {
    ...
    it('+ Fails for a non-existent user', function (done) {
      ...
      request.get('/disconnect')
        ...
        .end((requestErr, res) => {
          ...
          expect(res.body).to.deep.eql({ error: 'Unauthorized' }); // Validate error for invalid token
          done();
        });
    });
    ...
    it('+ Succeeds with a valid "X-Token" field', function (done) {
      request.get('/disconnect')
        .set('X-Token', token)
        .expect(204) // Validate successful disconnection
        .end((err, res) => {
          if (err) {
            return done(err);
          }
